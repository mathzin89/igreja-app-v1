...
import autoTable from 'jspdf-autotable'; // Mantenha, mesmo que não usado diretamente no exemplo, se precisar.
...
export default function PaginaMembros() {
  ...
  const generateMemberFichaPdf = async (membro: Membro | null) => {
    ...
    const addSectionTitle = (title: string) => {
      ...
      doc.line(startX, currentY + 3, pageWidth - margin, currentY + 3);
    ...
    <Box sx={{ p: 3 }}>
      ...
      {!error && (
        <Paper sx={{ width: '100%', overflow: 'hidden' }}>
          {/* REVISÃO AQUI NO TableContainer e Table */}
          <TableContainer
            sx={{
              maxHeight: 640,
              width: '100%', // Garantir que ocupa 100% do pai Paper
              display: 'block', // Garantir que se comporta como um bloco para overflow
              overflowX: 'auto', // A barra de rolagem horizontal
              // Adicionar um border para visualizar a caixa da TableContainer
               //border: '1px solid red', // Descomente para depuração visual
            }}
          >
            <Table
              stickyHeader
            ...
            >
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: "bold", minWidth: 80 }}>Foto</TableCell>
                  <TableCell sx={{ fontWeight: "bold", minWidth: 150 }}>Nome</TableCell>
                  <TableCell sx={{ fontWeight: "bold", minWidth: 120 }}>Celular</TableCell>
                  <TableCell sx={{ fontWeight: "bold", minWidth: 120 }}>Cidade</TableCell>
                  <TableCell sx={{ fontWeight: "bold", minWidth: 120 }}>Congregação</TableCell>
                  <TableCell sx={{ fontWeight: "bold", minWidth: 100 }}>Cargo</TableCell>
                  <TableCell sx={{ fontWeight: "bold", minWidth: 80 }}>Status</TableCell>
                  <TableCell sx={{ fontWeight: "bold", minWidth: 180 }}>Ações</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {loading ? (
                  ...
                  <TableRow hover key={membro.id}>
                    <TableCell sx={{ minWidth: 80 }}><Avatar src={membro.foto}>{membro.nome.charAt(0)}</Avatar></TableCell>
                    <TableCell sx={{ minWidth: 150 }}>{membro.nome}</TableCell>
                    <TableCell sx={{ minWidth: 120 }}>{membro.celular}</TableCell>
                    <TableCell sx={{ minWidth: 120 }}>{membro.cidade}</TableCell>
                    <TableCell sx={{ minWidth: 120 }}>{membro.congregacao}</TableCell>
                    <TableCell sx={{ minWidth: 100 }}>{membro.cargo}</TableCell>
                    <TableCell sx={{ minWidth: 80 }}><Chip label={membro.status} color={membro.status === "Ativo" ? "success" : "error"} size="small" /></TableCell>
      ...
      {/* MODAL DE ADIÇÃO, EDIÇÃO, VISUALIZAÇÃO E DELEÇÃO (MANTIDOS IGUAIS) */}
      {/* ... (código dos modais) */}
       {/* MODAL DE ADIÇÃO */}
